...
plusPH
    :: (Eq a, Fractional a) => PolyHeaviside a -> PolyHeaviside a -> PolyHeaviside a
...
comparePHs (lf, uf, (H x y, Ph f))
    | lf /= uf =
        error "Non-zero Heaviside interval"
...
instance (Fractional a, Eq a, Ord a) => Comparable a (PolyHeaviside a) where
...
-}
instance (Num a, Eq a, Fractional a) => Mergeable (PolyHeaviside a) where
...
-}
polyHeavisideRoot
    :: OrdNumEqFrac a => a -> a -> (a, a) -> PolyHeaviside a -> Maybe a
...
instance OrdNumEqFrac a => Displayable a (PolyHeaviside a) where
