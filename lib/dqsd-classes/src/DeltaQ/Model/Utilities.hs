...
class (DeltaQ irv) => DeltaQVisualisation irv where
    ...
    asDiscreteCDF
        :: irv -> Int -> [Either (Time irv, ProbMass irv) [(Time irv, ProbMass irv)]]
...
plotCDF title irv = execEC $ do
    ...
    layout_x_axis
        . laxis_generate
        .= maybe autoAxis (\u' -> scaledAxis def (0, factor * u')) u
    layout_y_axis . laxis_title .= "Prob. Mass"
    plot
        $ line "" [cdf ++ maybe [] (\u' -> [(2 * factor * u', tangibleMass irv)]) u]
  where
...
plotCDFs title irvs = execEC $ do
    ...
    layout_x_axis
        . laxis_generate
        .= maybe autoAxis (\u' -> scaledAxis def (0, factor * u')) maxSupport
    layout_y_axis . laxis_title .= "Prob. Mass"
    mapM_ plotOne irvs
  where
    ...
    plotOne (t, irv) =
        plot
            $ line
                t
                [ cdf irv 1000
                    ++ maybe [] (\u' -> [(2 * factor * u', tangibleMass irv)]) (snd $ support irv)
                ]
...
plotPDF title irv = execEC $ do
    ...
    plot
        $ line ""
        $ map (either (\(t, p) -> [(t, never), (t, p)]) id)
        $ asDiscretePDF irv 1000
  where
    factor = 1.1
    (_, u) = support irv
